package core.behaviorItems;
   ...
   *
   * @preConditions:
   * - The final target must be set using `setFinalTarget()` before activating the movement.
   ...
   * - Movement deactivates automatically when the total time exceeds t1 + t2 + t3.
   * - Upon deactivation:
   *   - Time (t) resets to 0.
   *   - Initial position, arc end position, circle center, and final approach start position are cleared.
   *   - Enemy velocity is set to (0, 0).
   *   - The enemy's final position matches the final target position.
  ...
  public class EnterOverTopMovement implements IEnemyMovement {
       ...
       *
       * @param goRight If true, the movement goes from right-to-left; otherwise, left-to-right.
       */
      ...
      /**
       * Updates the enemy's position and velocity based on the current time parameter `t`.
       * Progresses through the movement phases and deactivates the movement when completed.
       *
      ...
      public void move(GameObject enemy) {
          ...
          if (initialPosition == null) initialPosition = enemy.transform().position();
       ...
       * Determines the phase of the movement and delegates to the appropriate handler.
       *
       * @param localT The time parameter.
       * @return The position as a `Ponto` object.
       */
      private Ponto getPositionAtTime(double localT) {
          if (localT < t1) return handleArc(localT / t1);
